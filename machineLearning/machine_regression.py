...
class MachineLearningRegression:
    def __init__(self, data_pr, prediction_array=None, models=None):
        ...
        self.model_evaluvation_dict = {
            str(i).replace("()", ""): {"model_object": i} for i in models
        }
        self.model_prediction = {str(i).replace("()", ""): None for i in models}

    def fit(self):
        for model, _ in self.model_evaluvation_dict.items():
            self.model_evaluvation_dict[model]["model_object"].fit(
                self.train_features, self.train_target
            )
            self.trained_models.append(
                self.model_evaluvation_dict[model]["model_object"]
            )
            self.model_prediction[model] = self.model_evaluvation_dict[model][
                "model_object"
            ].predict(self.test_features)
    ...
    def Score_test_dataset(self):
        ...
        for model, _ in self.model_evaluvation_dict.items():
            for metric, obj in metrics.items():
                self.model_evaluvation_dict[model][metric] = obj(
                    self.model_prediction[model], self.test_target
                )
    ...
    def evaluvate(self):
        ...
        if isinstance(self.prediction_array, np.ndarray):
            self.model_evaluvation_dict["prediction"] = self.best_model[
                "model_obj"
